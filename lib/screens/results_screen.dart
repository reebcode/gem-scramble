...
/// Lightweight, self-contained confetti effect rendered with a [CustomPainter].
/// Particle layout is derived deterministically from a fixed seed so it stays
/// stable across the per-frame repaints driven by the results animation.
class _ConfettiPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < _particleCount; i++) {
      ...
      final drift = (random.nextDouble() - 0.5) * 160;
      final delay = random.nextDouble() * 0.3;
      final fallSpeed = 0.7 + random.nextDouble() * 0.6;
      final width = 5 + random.nextDouble() * 7;
      final height = 8 + random.nextDouble() * 10;
      final spin = (random.nextDouble() - 0.5) * 12;
      final color = _colors[i % _colors.length];

      final t = ((progress - delay) / (1 - delay)).clamp(0.0, 1.0);
      if (t <= 0) continue;

      final y = -height + t * (size.height + height * 2) * fallSpeed;
