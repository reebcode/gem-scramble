...
export async function tryDebitUser(
  ...
  try {
    const success = await prisma.$transaction(async (tx) => {
      ...
      const usedGems = Math.min(user.gemBalance, amountGems);
      let usedBonusGems = 0;
      ...
      // Atomic, guarded decrement: the WHERE clause requires the balances to
      // still cover the debit, so concurrent debits cannot drive a balance
      // negative (a stale read can no longer over-spend). If another request
      // won the race, count is 0 and we abort.
      const updated = await tx.user.updateMany({
        where: {
          id: user.id,
          gemBalance: { gte: usedGems },
          bonusGems: { gte: usedBonusGems },
        },
        data: {
          gemBalance: { decrement: usedGems },
          bonusGems: { decrement: usedBonusGems },
        },
      });
      if (updated.count === 0) return false;

      // Record the entry fee. The unique (userId, matchId, type) constraint
      // makes this idempotent; a duplicate throws P2002 and rolls back the
      // decrement above (handled below as "already debited").
