...
import { creditPrize, tryDebitUser } from "../services/database.js";
...
const joinSchema = z.object({
  ...
  // userId is only honored in dev mode; in Firebase mode the user is derived
  // from the verified token and any client-supplied userId is ignored.
...
router.post("/join", async (req, res) => {
  ...
  const { lobbyType } = parsed.data;

  try {
    // Authoritative identity: derived from the auth token (Firebase) or the
    // dev userId. Never trust a client-supplied userId in production.
    const requestingUser = await getRequestingUser(req);
    const userId = requestingUser.id;
    const playerName =
      parsed.data.playerName || requestingUser.username || "Player";
    const nowIso = new Date().toISOString();
    ...
    console.error(`Failed to join lobby ${lobbyType}:`, error);
...
router.post("/leave", async (req, res) => {
  ...
  try {
    ...
    // If no players left, delete the match entirely
    if (activeMatch.players.length === 0) {
      await repo().clearWaiting(activeMatch.lobbyType);
      await repo().remove(activeMatch.id);
    } else {
...
const submitSchema = z.object({
  userId: z.string().min(1).optional(),
  words: z.array(z.string().max(40)).max(500).default([]),
});
...
router.get("/:matchId/details", async (req, res) => {
  try {
    ...
    const matchDetails = {
      ...
      maxPlayers: lobbyConfig?.maxPlayers ?? m.players.length,
