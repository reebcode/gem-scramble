...
import { getAuthMode } from "./config/auth.js";
import { buildCorsMiddleware, getAllowedOrigins } from "./config/cors.js";
...
import gems from "./modules/gems.js";
...
const app = express();
// Behind a single proxy on most PaaS hosts (Railway, etc.) so rate-limit and
// logging see the real client IP.
app.set("trust proxy", 1);

// CORS must run before helmet and rate limiting so preflight OPTIONS succeed.
...
// Basic abuse protection across all endpoints.
app.use(
  rateLimit({
    windowMs: 60_000,
...
app.use("/gems", authFirebase, gems);
...
// Error handler. Log full detail server-side, but never leak internal error
// messages/stacks to clients (avoid info disclosure).
